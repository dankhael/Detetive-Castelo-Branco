function remove_item(_item_key){
	...
	if (!is_string(_item_key) || !struct_exists(global.item_list, _item_key) and !inventory_has_item(_item_key)) {
	...
	var item_index = array_get_index(obj_item_manager.inv, item_data_completo);
	
	// Adiciona a STRUCT COMPLETA ao array do inventário
	array_delete(obj_item_manager.inv,  item_index, 1);
	
	// Esta linha agora é segura e não vai mais quebrar o jogo.
	show_debug_message("Item '" + item_data_completo.name + "' foi removido do inventário.");
}